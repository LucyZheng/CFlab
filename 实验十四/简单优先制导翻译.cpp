...
#include<string>
using namespace std;
struct { int Nv; string VN[100]; } Vns = { 0 };
struct { int Nt; string VT[100]; } Vts = { 0 };
struct { int Np; string PL[200], PR[200]; bool enabled[200]; int action[200]; } ps = { 0 };
string S;
int equalm[100][100];
string act[200];
...
int main() {
	...
	for (int i = 0; i < ps.Np; i++)
		cin >> tmp >> ps.PL[i] >> ps.PR[i]>>ps.action[i];
	int acnum;
	cin >> acnum;
				...
				for (int i = len2 - 1; i > 0; i--) {
					if (output[i - 1] == '#') {
						...
						for (int j = 0; j < ps.Np; j++) {
							if (tmp == ps.PR[j]) {
								if (ps.action[j] != 0) cout << " " << act[ps.action[j]-1];
								output += ps.PL[j];
								break;
							}
						}
